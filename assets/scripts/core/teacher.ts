...
const TEACHER_SCRIPTS: Record<string, string> = {
  ...
  'level-4': '第一次打怪！先走到敌人旁边，再用 attack() 打它。它 20 点血，要砍两刀。',
  'level-5': '宝石不用特意捡，走过去就自动捡起来了。',
  'level-6': '小兽 10 点血一刀就倒；食人魔 20 点血要两刀。打完一个再看下一个。',
  'level-7': '路要转弯：先数横着走几格，再数竖着走几格。',
  'level-8': '药水走过去就自动喝，能回 25 点血。先喝药水再打怪更安全。',
  'level-9': '这一关要写 17 行移动，手会有点酸。下一章教你偷懒的办法。',
  'level-10': '把学过的都用上：走路、打怪、捡宝石，最后走到出口。',
